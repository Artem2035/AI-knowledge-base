...
**LLM-провайдер по умолчанию — Groq Free Tier** (`llm_provider="groq"` в `config/settings.py`, см. §5). Gemini Free Tier остаётся полностью поддерживаемой альтернативой — переключение через `LLM_PROVIDER=gemini` в `.env`, без изменений в коде ролей (`llm/factory.py` — единственная точка выбора, `llm/base.py::LLMClient` — общий Protocol для обоих клиентов). Часть внутренних идентификаторов в коде исторически называется "Gemini" (`GeminiBudget`, `GeminiFreeLimitReached`, переменная `MAX_GEMINI_CALLS_PER_TASK`, пакет `gemini/`) — это осталось с более ранней версии, когда Gemini был единственным провайдером; функционально они одинаково применяются к обоим провайдерам (см. комментарий в `orchestrator/state_machine.py`: *"Имя атрибута исторически 'gemini', но фактический тип определяется settings.llm_provider"*).
...
┌──────────────────────────────────────────────────────────────────┐
│                         ORCHESTRATOR                              │
│  state machine (не LLM) + бюджет LLM-вызовов + error handling    │
└───────────────────────────────┬──────────────────────────────────┘
      │                         │                         │
      ▼                         ▼                         ▼
┌────────────┐          ┌───────────────┐         ┌───────────────┐
│    LLM     │◄─────────┤   ROLE LAYER   ├────────►│  LOCAL TOOLS  │
│  CLIENT    │  prompts  │ (промпт-режимы│  calls  │ (детерминизм) │
│ (Groq по   │           │  одной модели)│         │               │
│ умолчанию, │           └───────────────┘         └───────┬───────┘
│ Gemini —   │                                              │
│  опция)    │                                              │
└────────────┘                                             │
...
Важно: **LLM (Groq Free Tier по умолчанию, Gemini Free Tier — опция) — reasoning engine, локальный код — execution layer, Vault — knowledge layer**. Ни один "агент" не имеет собственного независимого цикла — все вызовы инициирует Orchestrator по жёсткому конечному автомату (finite state machine), а не автономный agent loop.

---

## 2. Роли: что реально требует LLM (Groq по умолчанию), а что — нет
...
- **`web`** (прежний путь) — Researcher ищет источники в интернете (DuckDuckGo, бесплатно) и скачивает их текст; Extractor+Critic извлекает evidence из РЕАЛЬНОГО текста источников. Даёт проверяемые `source_refs` в каждой заметке, но зависит от нестабильного сетевого I/O (поиск/скрейпинг могут падать, тормозить, блокироваться сайтами) и тратит больше LLM-вызовов (Groq/Gemini).
...
| 6 | **Vault Analyst** | Нет (LLM не нужен для 90% работы) | Локальный индекс (SQLite) + деterministic поиск (BM25/keyword) + embedding-опция (см. §6). Только финальное "это та же концепция или нет?" на пограничных случаях — 1 маленький вызов LLM (Groq) на кандидата-дубликат |
...
## 5. Как работает Free Tier LLM-провайдеров и как под него подстроиться

### 5.1 Groq Free Tier (провайдер по умолчанию)
...
Практически значимое отличие от Gemini (§5.2): у Groq **TPM (8 000 токенов/мин) — самое узкое место**, а не RPM или RPD — при системном промпте + JSON Schema уже в несколько сотен токенов и evidence-контексте на заметку легко подойти к пределу одного вызова. Поэтому `llm/groq_client.py::TokenRateLimiter` — не опциональная деталь, а центральный механизм клиента: sliding-window + leaky-bucket по TPM, адаптивная калибровка оценки токенов по роли (`TokenEstimateCalibrator`), учёт Groq prompt caching (кэшированные токены не считаются в лимит) и adaptive safety margin после реального `429`. См. подробный докстринг `llm/groq_client.py` — там описаны все итерации этой логики и почему.

RPD сбрасывается по UTC согласно политике Groq (не Pacific Time, как у Gemini — см. §5.2). Лимиты — на аккаунт/API-key, не суммируются между ключами (создание второго ключа "для обхода лимита" — прямое нарушение требования FREE_ONLY / никакого искусственного скейлинга, поэтому в MVP не делаем).

### 5.2 Gemini Free Tier (альтернативный провайдер, `LLM_PROVIDER=gemini`)

- Лимиты считаются по **трём измерениям одновременно**: RPM, TPM, RPD — превышение любого из них даёт `429`.
...
- Ориентировочно: Flash ~10 RPM / ~250–1500 RPD, Flash-Lite ~15 RPM / выше RPD. Это **не гарантированные цифры** — система должна читать реальный лимит с ошибки API, а не хардкодить.
- RPD сбрасывается в полночь по Pacific Time.
- В отличие от Groq, TPM у Gemini заметно шире — `gemini/client.py` поэтому не реализует собственный TPM-лимитер уровня `TokenRateLimiter`, полагаясь на retry с backoff при `429`.

### 5.3 Как система с этим работает (общая логика для обоих провайдеров)

1. **GroqClient** (по умолчанию) / **GeminiClient** (опция) — единственная точка входа к LLM для своего провайдера. Все вызовы идут только через `llm/factory.py::create_llm_client`, который выбирает нужный клиент по `settings.llm_provider`; `roles/*` работают с общим Protocol `llm/base.py::LLMClient` и не знают, какой провайдер активен.
2. Перед каждым вызовом — проверка локального счётчика (`calls_today`, `calls_this_minute`) относительно конфигурируемых мягких лимитов (`groq_rpm_soft_limit`/`groq_rpd_soft_limit` или `gemini_rpm_soft_limit`/`gemini_rpd_soft_limit`) — это **best-effort throttle**, не источник истины (источник истины — реальный ответ API, см. докстринг `orchestrator/budget.py`).
...
5. Глобальный **бюджет вызовов на задачу** (`MAX_GEMINI_CALLS_PER_TASK`, например 40; имя переменной исторически привязано к Gemini, но действует одинаково для обоих провайдеров) — жёсткий потолок независимо от лимитов API, чтобы не было "тихого" разрастания количества вызовов при большой теме. В RESEARCH_MODE=knowledge реальный расход обычно заметно ниже потолка (см. §2.2) — потолок остаётся общим "предохранителем" на случай богатой темы с большим числом заметок и критик-раундов.
6. Для роли Extractor/Elaborator (самая частая по числу вызовов, см. §2.2) используется ОТДЕЛЬНЫЙ клиент/бюджет (`orchestrator/state_machine.py::self.extraction_client`) — на Groq это позволяет использовать модель с другим TPM-профилем (`groq_extraction_model`/`groq_extraction_tpm_limit`) независимо от модели остальных ролей; на Gemini это тот же клиент, что и для прочих ролей (отдельной "extraction"-модели с другим TPM нет смысла заводить).
...
| LLM client | `openai` SDK (Groq, OpenAI-совместимый base_url, дефолт) / `google-genai` (Gemini, опция) | Structured output (JSON schema/json_object), retry-friendly интерфейс; для Groq — дополнительно клиентский TPM-лимитер (см. §5.1) |
...
│   └── budget.py            # контроль числа LLM calls / rate limit (общий для Groq и Gemini)
├── gemini/
│   ├── client.py            # единственная точка вызова Gemini API (альтернативный провайдер)
│   ├── schemas.py           # Pydantic-схемы structured output по ролям
│   └── prompts/             # статические системные инструкции ролей
│       ├── planner.py
│       ├── vault_analyst.py
│       ├── synthesizer_writer.py
│       ├── elaborator.py    # NEW — RESEARCH_MODE=knowledge
│       └── critic.py        # NEW — ревью написанной заметки
├── llm/
│   ├── base.py               # Protocol LLMClient
│   ├── factory.py            # выбор провайдера (gemini/groq)
│   ├── groq_client.py
...
│   ├── elaborator.py          # NEW — активен только в RESEARCH_MODE=knowledge
│   ├── vault_analyst.py
│   ├── synthesizer_writer.py
│   └── critic.py               # NEW — ревью написанной заметки, оба режима
...
│   ├── test_validation/
│   ├── test_staging/
│   ├── test_elaborator.py     # NEW
│   ├── test_critic.py          # NEW
│   └── test_gemini_mocked/    # моки Groq/Gemini для тестов без реальных вызовов
├── docs/
...
- **LLM**: Groq Free Tier (дефолт, см. §5.1) или Gemini Free Tier (Flash/Flash-Lite, опция, см. §5.2) — единственный обязательный внешний сервис.
...
3. **RPM/RPD/TPM Groq/Gemini Free** — на "богатую" тему с 8+ подтемами и множеством заметок можно упереться в дневной лимит (особенно в TPM у Groq, см. §5.1); задача должна уметь **приостанавливаться и продолжаться на следующий день** без потери прогресса (это заложено в дизайне staging/checkpoint).
